...
static void enumerate_usb_mass_storage(struct udev* udev)
{
    ...
    udev_list_entry_foreach(entry, devices) {
        ...
        if (block && scsi_disk && usb) {
            printf("iSerialNumber=%s IdProduct = %s, idVender = %s bMaxPacketSize=%d, scsi = %s\n",
                   udev_device_get_devnode(block),
                   udev_device_get_sysattr_value(usb, "iSerialNumber"),
				   udev_device_get_sysattr_value(usb, "idProduct"),
                   udev_device_get_sysattr_value(usb, "idVendor"),
                   udev_device_get_sysattr_value(usb, "bMaxPacketSize0"),
